function fieldStats=runFieldStatistics(user,row,mode)
% P. Bauer 2020
...
if exist(loadpath,'file')
    load(loadpath,'dFF_out','meandFFs','centroidrho')
    size(dFF_out)
    for lap=1:size(dFF_out,1)
        if centroidrho(lap)>400
        plot(meandFFs{lap}(2:end))
        hold on
        end
    end
else
